...
void draw()
{
  ...
  if (mouseX > 750 && mouseX < 1175 && mouseY > 175 && mouseY < 925) {
    if (isMouseReleased == true) {
      if (menuClick.isPlaying() == false) {
        menuClick.play();}
        sceneLoader = 11;
    }
  }
...
>>>>>>> Stashed changes
  ...
  if (sceneLoader == 11) {
    if (lightState >= 40) {
      image (WaitingRoom,0,0); neonBuzz.pause();}
    else {
      image(WaitingRoom,0,0); neonBuzz.loop(1,0.1);}
  if (mouseX > 0 && mouseX < 1920 &&mouseY > 900 && mouseY < 1080) {
    if (isMouseReleased == true) {
    if(menuClick.isPlaying()== false) {menuClick.play();}
  whatText = 0;
